	...
	async run(message, args) {
		...
		if (url.match(/^(https?:\/\/)?(www\.)?(m\.)?(youtube\.com|youtu\.?be)\/.+$/gi)) {
			...
			try {
				...
				if (searched.videos.length === 0) return message.chhanel.send('Looks like i was unable to find the song on YouTube', message.channel);
		...
		const play = async (song) => {
			...
			if (!song) {
				if (!online.afk) {
					...
					message.channel.send('Leaving the voice channel because I think there are no songs in the queue. If you like the bot stay 24/7 in voice channel run `!afk`\n\nThank you for using my code! [GitHub](https://github.com/SudhanPlayz/Discord-MusicBot)');
